...
func NewReverse(ms uint64) (id ULID, err error) {
	_ms := ms ^ MaxTime()

	//entropy := ZeroReader{Nonce: math.MaxUint64 - nonce, From: addr}
...
func (id ULID) TimeString() string {
	dst := make([]byte, 10)
...
func (z ZeroReader) Read(p []byte) (int, error) {
	binary.BigEndian.PutUint64(p[0:], z.Nonce)
	// for i := range p {
	// 	p[i] = 255
	// }
	return len(p), nil
